...
Wer das System [_Overleaf_](https://www.overleaf.com) nutzt, kann dieses `ZIP`-File in Overleaf als neues Projekt installieren. Die Vorlage habe ich dort getestet und es hat alles funktioniert.

Und bitte beachten: In dem Menüpunkt `Recompile` den Unterpunkt `Stop on first error` und dann den Fehler korrigieren. Dazu wird durch `Overleaf` angezeigt, in welcher Textzeile sich dieser findet. Für die Endredaktion sind fehlerfreie Manuskripte erforderlich.
